package by.bsuir.mpp.xpulse.security;
...
public class DomainUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(final String login) {
        ...
        Optional<User> userByEmailFromDatabase = userRepository.findOneWithAuthorityByEmail(lowercaseLogin);
        return userByEmailFromDatabase.map(user -> createSpringSecurityUser(lowercaseLogin, user)).orElseGet(() -> {
            Optional<User> userByLoginFromDatabase = userRepository.findOneWithAuthorityByLogin(lowercaseLogin);
